        ...
        if (accountManager.addAccountExplicitly(account, null, null)) {
            ...
            if (ContentResolver.getPeriodicSyncs(account, authority).isEmpty()) {
                String strFrequency = PreferenceManager.getDefaultSharedPreferences(context)
                        .getString(PREF_ACCOUNT_SYNC_FREQUENCY, "-1");
                int frequency = Integer.parseInt(strFrequency);
                if (frequency > 0) {
                    Timber.d("Account already existed, frequency was set to %s", frequency);
                    startPeriodicSync(context, account, frequency);
                } else {
                    Timber.d("Account already existed, frequency not set because of preferences");
                }
            } else {
                Timber.d("Account not created.... ");
                NotificationCreator.createNotificationWithDevMessage(context, "Sync Account was not Created");
            }
        }
