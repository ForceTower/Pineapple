...
public class SyncJobScheduler extends JobService {
    ...
    public boolean onStartJob(JobParameters params) {
        ...
        try {
            ...
            executors.diskIO().execute(() -> {
                ...
                long id = uDatabase.syncRegistryDao().insert(registry);
                registry.setUid(Long.valueOf(id).intValue());
                initiateSync();
            });
        } catch (Exception e) {
    ...
    public boolean onStopJob(JobParameters params) {
        Timber.d("System Says you should stop running");
        return false;
    }
    ...
    private void updateAccountObserver(ApiResponse<UpdateStatus> updateResp) {
        ...
        if (!status.isWorker()) {
            ...
            executors.diskIO().execute(() -> uDatabase.syncRegistryDao().insert(registry));
    ...
    private void proceedSync() {
        Timber.d("Application is online [JOB]");
        executors.diskIO().execute(() -> {
            try {
